...
.text
	...
	jal jugador
	jal enemigos
	#guardamos las pos de los enemigos generados
	sw $t6, enemy1_x
	sw $t7, enemy1_y
	jal enemigos
	sw $t6, enemy2_x
...
#las funciones de move se basan en dicha posicion para la suma y resta de posiciones
	...
	move_up:
		la $s0, entity_x	#para reutilizar la funcion, entity sera jugador1, enemy1, enemy2, enemy3
		la $s1, entity_y
	...
	move_right:
		...
		sw $t0, 0($s0)
		j update_entity
	#funcion para saber que debe ser entity acorde al momento de gameplay en el que estemos
	update_entity:
	lw $t4, current_entity
	...
	#las funciones update es solo para asignar la entidad con la que se va a trabajar en los word entity
	...
	#si la entidad es 3, reiniciamos a entidad 0 (jugador)
	...
	check_color_at_position:
		...
		#verificamos la esquina inferior derecha
		la $s3, jugador_x
		la $s4, jugador_y
		lw $s3, 0($s3)
		lw $s4, 0($s4)
		...
		#verificamos la esquina inferior izquierda
		la $s3, jugador_x
		la $s4, jugador_y
		lw $s3, 0($s3)
		lw $s4, 0($s4)
		...
		#verificamos la esquina superior izquierda
		la $s3, jugador_x
		la $s4, jugador_y
		lw $s3, 0($s3)
		lw $s4, 0($s4)
		...
		#verificamos la esquina superior derecha
		la $s3, jugador_x
		la $s4, jugador_y
		lw $s3, 0($s3)
		lw $s4, 0($s4)
